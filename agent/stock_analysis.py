...
# This tool uses OpenRouter's function calling feature to parse user queries
# and extract structured data like stock symbols, investment amounts, etc.
extract_relevant_data_from_user_prompt = {
    "type": "function",  # Required field for OpenRouter/OpenAI-compatible function calling
    "function": {
...
class StockAnalysisFlow(Flow):
    ...
    async def chat(self):
        ...
        - Use OpenRouter to analyze the user's message and extract structured data
        ...
        try:
            ...
            # Step 2.3: Call OpenRouter to analyze user input and extract investment data
            model = OpenAI(
            ...
            # Step 2.5: Check if OpenRouter extracted structured data via function calling
            if(response.choices[0].finish_reason == "tool_calls"):
                ...
                # Add tool message response for each tool call (required by OpenRouter/OpenAI-compatible API)
    ...
    async def insights(self):
        ...
        - Use OpenRouter to generate positive and negative analysis
...
def convert_tool_call(tc):
    """
    Utility function to convert OpenRouter/OpenAI-compatible tool call format to our internal format
    
    Args:
        tc: OpenRouter/OpenAI-compatible tool call object
